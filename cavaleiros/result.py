...
class Result(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        self.drawMapa(qp)
        qp.drawPixmap(
            37 * 16, 4 * 16, QtGui.QPixmap(os.path.join(self.image, "athena.png"))
        )
        self.drawCava(qp)
        self.placarLife(qp)
        self.placarTime(qp)
    ...
    def moviCava(self):
        if self.checaStatu():
            ...
            if (
                self.mapa.mapa[self.caminho[self.novo].v_x][self.caminho[self.novo].v_y]
                != 3
            ):
                self.mapa.mapa[self.caminho[self.novo].v_x][
                    self.caminho[self.novo].v_y
                ] = 4
            self.novo += 1
    ...
    def checaStatu(self):
        ...
        if self.player.pos_x == 4 and self.player.pos_y == 37:
            ...
            self.player.pos_x == self.casas[self.casa_at].pos_x
            and self.player.pos_y == self.casas[self.casa_at].pos_y
        ):
            self.battle = True
            self.casa_at += 1
            self.novo = 0
            ...
            for j in range(self.mapa.alt):
                ...
                if self.mapa.mapa[i][j] == 3:
                    qp.drawPixmap(
                        j * 16,
                        i * 16,
                        QtGui.QPixmap(os.path.join(self.image, "templo.png")),
                    )

    def drawCava(self, qp):
        qp.drawPixmap(
            self.player.pos_y * 16,
            self.player.pos_x * 16,
            QtGui.QPixmap(os.path.join(self.image, "seya.png")),
        )
    ...
    def placarLife(self, qp):
        if self.cavaleiros[0].vida == 0:
            qp.drawPixmap(
                42 * 16,
                0 * 16,
                QtGui.QPixmap(os.path.join(self.image, "seya2_rip.png")),
            )
        else:
            qp.drawPixmap(
                42 * 16, 0 * 16, QtGui.QPixmap(os.path.join(self.image, "seya2.png"))
            )
        for i in range(self.cavaleiros[0].vida):
            qp.drawPixmap(
                (42 + i) * 16,
                5 * 16,
                QtGui.QPixmap(os.path.join(self.image, "vida.png")),
            )
        ...
        if self.cavaleiros[1].vida == 0:
            qp.drawPixmap(
                42 * 16,
                7 * 16,
                QtGui.QPixmap(os.path.join(self.image, "shiryu_rip.png")),
            )
        else:
            qp.drawPixmap(
                42 * 16, 7 * 16, QtGui.QPixmap(os.path.join(self.image, "shiryu.png"))
            )
        ...
        if self.cavaleiros[2].vida == 0:
            qp.drawPixmap(
                42 * 16,
                14 * 16,
                QtGui.QPixmap(os.path.join(self.image, "hyoga_rip.png")),
            )
        else:
            qp.drawPixmap(
                42 * 16, 14 * 16, QtGui.QPixmap(os.path.join(self.image, "hyoga.png"))
            )
        ...
        if self.cavaleiros[3].vida == 0:
            qp.drawPixmap(
                42 * 16,
                21 * 16,
                QtGui.QPixmap(os.path.join(self.image, "shun_rip.png")),
            )
        else:
            qp.drawPixmap(
                42 * 16, 21 * 16, QtGui.QPixmap(os.path.join(self.image, "shun.png"))
            )
        ...
        if self.cavaleiros[4].vida == 0:
            qp.drawPixmap(
                43 * 16,
                27.7 * 16,
                QtGui.QPixmap(os.path.join(self.image, "ikki_rip.png")),
            )
        else:
            qp.drawPixmap(
                43 * 16, 27.7 * 16, QtGui.QPixmap(os.path.join(self.image, "ikki.png"))
            )
    ...
    def drawBattle(self, qp, cava, seque):
        ...
        qp.drawPixmap(
            16 * 16,
            13 * 16,
            QtGui.QPixmap(os.path.join(self.image, self.casas[self.casa_at - 1].title)),
        )
        qp.drawPixmap(
            cs * 16,
            15 * 16,
            QtGui.QPixmap(
                os.path.join(self.image, self.casas[self.casa_at - 1].cava_or)
            ),
        )
        qp.drawPixmap(
            vs * 16, 20 * 16, QtGui.QPixmap(os.path.join(self.image, "VS2.png"))
        )
        qp.drawPixmap(
            cv * 16,
            15 * 16,
            QtGui.QPixmap(os.path.join(self.image, self.cavaleiros[seque[0]].cava)),
        )
        ...
        for i in seque:
            ...
            qp.drawPixmap(
                cv * 16,
                15 * 16,
                QtGui.QPixmap(os.path.join(self.image, self.cavaleiros[i].cava)),
            )
